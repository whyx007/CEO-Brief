...
from shared.utils.time_filter import filter_by_date as _filter_by_date, filter_by_date_strict as _filter_by_date_strict
...
class FreeNewsPipeline:
    ...
    def collect_rss(self, limit_per_feed: int = 5, extra_sources: list[dict[str, Any]] | None = None) -> dict[str, Any]:
        ...
        def _fetch_one(source: dict[str, Any]) -> tuple[dict[str, Any], list[dict[str, Any]]]:
            remote_timeout = float(os.getenv('RSS_REQUEST_TIMEOUT_SECONDS', '8'))
            if 'RSSHub ' in source.get('name', ''):
                remote_timeout = float(os.getenv('RSSHUB_TIMEOUT_SECONDS', '8'))
            try:
        ...
        # 2天时间窗口过滤
        items = _filter_by_date(items, max_days=2)
    ...
    def collect_policy_news(self, target_settings: dict[str, Any], limit_per_query: int = 3, max_days: int = 2) -> dict[str, Any]:
        ...
        # 时政严格时间窗口过滤：超2天或无法解析日期的条目直接丢弃
        filtered = _filter_by_date_strict(filtered, max_days=max_days)
    ...
    def merge_and_dedup(self, *item_groups: list[dict[str, Any]], max_days: int = 2) -> dict[str, Any]:
        ...
        for group in item_groups:
            for item in group or []:
                ...
                seen_urls.add(url)
                merged.append(item)
        # 2天时间窗口过滤：无法解析日期的条目保留（容错）
        merged = _filter_by_date(merged, max_days=max_days)
